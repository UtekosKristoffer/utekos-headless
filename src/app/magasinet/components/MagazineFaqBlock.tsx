    ...
    <section className='my-16'>
      ...
      <dl className='mt-8 space-y-4'>
        {block.items.map(item => (
          <div key={item.question} className='rounded-lg border border-background/10 bg-cloud-dancer p-5'>
            <dt className='font-google-sans text-xl font-bold leading-[1.05]   text-background'>
              {item.question}
            </dt>
            <dd className='mt-3   text-base leading-[1.55]   text-background/76'>{item.answer}</dd>
          </div>
        ))}
      </dl>
    </section>
