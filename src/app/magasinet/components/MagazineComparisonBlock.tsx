    ...
    <section className='my-16'>
      ...
      <div className='mt-8 grid grid-cols-1 gap-4 lg:grid-cols-3'>
        {block.columns.map(column => (
          <section key={column.title} className='rounded-lg border border-background/10 bg-cloud-dancer p-5'>
            <h3 className='font-google-sans text-2xl font-bold leading-[0.95]   text-background'>
              <MagazineInlineTitle text={column.title} />
            </h3>
            {column.text && (
              <p className='mt-3   text-base leading-[1.55]   text-background/74'>{column.text}</p>
            )}
