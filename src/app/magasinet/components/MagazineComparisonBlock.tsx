      ...
      <div className='mt-8 grid grid-cols-1 gap-4 lg:grid-cols-3'>
        {block.columns.map(column => (
          ...
          >
            ...
            {column.text && (
              <p className='mt-3 font-utekos-text text-base leading-[1.55] tracking-tight text-maritime-darkest/74'>
                {column.text}
              </p>
            )}
            <ul className='mt-5 space-y-3'>
              {column.items.map(item => (
                <li
                  key={item}
                  className='flex gap-3 font-utekos-text text-sm leading-[1.45] tracking-tight text-maritime-darkest/80'
                >
