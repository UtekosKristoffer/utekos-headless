...
const calloutClassByTone = {
  ...
  commerce: 'border-primary-button/30 bg-primary-button text-maritime-darkest'
...
export function MagazineCalloutBlock({ block }: MagazineCalloutBlockProps) {
  return (
    <aside className={`my-14 rounded-lg border p-6 sm:p-8 ${calloutClassByTone[block.tone]}`}>
      {block.title && (
        <h3 className='font-google-sans text-2xl font-bold leading-[0.95] tracking-tight sm:text-3xl'>
          <MagazineInlineTitle text={block.title} />
        </h3>
      )}
      <p className='mt-4 font-utekos-text text-lg leading-[1.55] tracking-tight opacity-90'>
        {block.text}
      </p>
    </aside>
  )
