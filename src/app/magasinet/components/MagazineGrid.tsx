...
function FeaturedArticleCard({ article }: { article: MagazineArticle }) {
  return (
    <section className='bg-overcast py-14 text-background sm:py-20'>
...
function MagazineArticleCard({ article }: { article: MagazineArticle }) {
  return (
    <li>
      <Link
        href={`/magasinet/${article.slug}` as Route}
        className='group block h-full overflow-hidden rounded-lg border border-background/10 bg-cloud-dancer transition-transform duration-300 hover:-translate-y-0.5 motion-reduce:transition-none motion-reduce:hover:translate-y-0'
...
export function MagazineGrid({ articles }: MagazineGridProps) {
  ...
  if (!featuredArticle) {
    return (
      <section className='bg-overcast py-16 text-background'>
        <div className='container mx-auto px-4'>
  ...
  return (
    <>
      ...
      <section
        className='bg-overcast pb-16 text-background sm:pb-24'
