...
export function BeredskapsProductGridCard({
  product,
  isPriority = false,
  initialOptions
  ...
  return (
    <Card className='group relative flex h-full flex-col overflow-hidden border-none bg-transparent shadow-none'>
      ...
      <CardContent className='flex-grow p-0 pt-4'>
        <Link href={productUrl} aria-label={`Se produkt ${product.title}`}>
          <h3 className='truncate text-center mx-auto font-medium'>
            {product.title}
          </h3>
        </Link>
        <p className='text-muted-foreground text-center mx-auto'>{price}</p>
      </CardContent>
    </Card>
  )
}
