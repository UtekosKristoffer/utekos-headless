...
export function ProductSpecsView({ technologyGroups }: { technologyGroups: readonly TechnologyGroup[] }) {
  const [activeTech, setActiveTech] = useState(technologyGroups?.[0]?.technologies?.[0]?.title || '')
      ...
      <div className='space-y-24 pb-24'>
        {technologyGroups.map(group => (
          <section key={group.groupTitle}>
            <h2 className='mb-8 border-b border-white/10 pb-4 text-sm font-bold tracking-widest text-foreground/90'>
              {group.groupTitle}
            </h2>
            <div className='space-y-8'>
              {group.technologies.map(tech => (
                ...
                >
                  <TechnologyBlock tech={tech} isActive={activeTech === tech.title} />
                </div>
              ))}
            </div>
          </section>
