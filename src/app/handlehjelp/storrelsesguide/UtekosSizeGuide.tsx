            ...
            <h2 className='text-3xl font-bold leading-[1.05]   sm:text-5xl'>
              <span className='inline-flex items-baseline'>
                <UtekosWordmark
                  aria-hidden='true'
                  focusable='false'
                  className='inline-block h-[0.82em] w-auto translate-y-[0.06em] align-baseline text-background'
                />
                <span className='sr-only'>Utekos</span>
              </span>{' '}
              - en unik tilnærming til passform
            </h2>
            <p className='mx-auto mt-5 max-w-2xl utekos-section-lead text-background/90'>
            ...
            <div className='mt-8 grid grid-cols-1 gap-12 sm:grid-cols-3'>
              {features.map(feature => (
                <div key={feature.title} className='flex flex-col items-center gap-4'>
                  <div className='flex size-12 items-center justify-center rounded-full bg-primary text-background'>
                    <feature.Icon className='size-6' aria-hidden='true' />
                  </div>
                  <div className='text-center'>
                    <p className='font-semibold'>{feature.title}</p>
                    <p className='mt-1 text-sm text-background/74'>{feature.description}</p>
                  </div>
                </div>
              ))}
            </div>
                    ...
                    <tbody className='divide-y divide-cloud-dancer/12'>
                      {utekosData.map(item => (
                        <tr key={item.measurement} className='hover:bg-cloud-dancer/50 transition-colors'>
                          <td className='whitespace-nowrap py-4 pl-4 pr-3 text-sm text-left font-semibold text-background sm:pl-6'>
                            {item.measurement}
                          </td>
                          <td className='whitespace-nowrap px-3 py-4 text-sm font-medium text-background text-center'>
                            {item.m}
                          </td>
                          <td className='whitespace-nowrap px-3 py-4 text-sm font-medium text-background text-center'>
                            {item.l}
