...
import type { InspirationSeasonDefinition, InspirationSeasonIconName } from '../theme/seasons'
...
function getSeasonIcon(iconName: InspirationSeasonIconName): LucideIcon {
...
export function InspirationSeasonsTabs({
  ...
  const roundedTriggerClassName =
    'inspiration-seasons-tab-trigger relative flex !h-auto min-h-20 w-full flex-col items-center justify-center overflow-hidden rounded-xl border border-cloud-dancer/12 bg-background/58 px-3 py-3 transition-all duration-300 hover:border-cloud-dancer/28 hover:bg-background/72 data-active:border-cloud-dancer/24 data-active:bg-primary data-active:text-background'

  const pillTriggerClassName =
    'relative flex !h-auto w-full items-center justify-center overflow-hidden rounded-lg border border-cloud-dancer/12 bg-background/58 px-3 py-3 transition-all duration-300 hover:border-cloud-dancer/28 data-active:border-cloud-dancer/18 data-active:bg-background data-active:text-cloud-dancer'
    ...
    >
      <TabsList className={listClassName}>
        {seasons.map(season => {
          ...
          const activeClassName = tabActiveClassName ?? 'text-cloud-dancer'
          const inactiveClassName = tabInactiveClassName ?? 'text-ancient-water'
          ...
          return (
            <TabsTrigger
              ...
              value={season.value}
              className={cn(
                variant === 'pill' ? pillTriggerClassName : roundedTriggerClassName,
                tabTriggerClassName
              )}
            >
                  ...
                  className={cn(
                    ...
                    variant === 'pill' ?
                      [
                        ...
                        isActive ? 'text-cloud-dancer' : 'text-muted-foreground'
                      ]
                    : [
                        'font-google-sans font-bold tracking-[-0.01em]',
      ...
      {seasons.map(season => {
        ...
        return (
          <TabsContent
            key={season.value}
            value={season.value}
            className='mt-0 min-w-0 w-full data-ending-style:hidden'
          >
            <div className='inspiration-seasons-tab-content-enter'>
              <Card
                className={cn(
                  'relative overflow-hidden rounded-2xl border-cloud-dancer/12 bg-background/58 py-0',
                  contentCardClassName
                )}
              >
                {showCardGlow ?
                ...
                <CardContent
                  className={
                    showTips ?
                      'relative grid gap-8 p-6 sm:p-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-start lg:gap-12 lg:p-10'
                ...
                >
                  <div>
                    <div className='mb-5 flex items-center gap-4'>
                      <div
                        className={cn(
                          'flex size-12 shrink-0 items-center justify-center rounded-xl border border-cloud-dancer/18 bg-background transition-shadow duration-300',
                          contentIconClassName
                        )}
                        style={showCardGlow ? { boxShadow: `0 0 20px ${season.glowColor}20` } : undefined}
                      >
                        <Icon className={cn('size-6', contentIconGlyphClassName ?? season.iconColor)} />
                      </div>
                      <h3
                        className={cn(
                          'font-google-sans text-2xl font-bold leading-none tracking-[-0.01em] text-cloud-dancer sm:text-3xl',
                          contentTitleClassName
                        )}
                      >
                    ...
                    <p
                      className={cn(
                        'text-lg leading-text-paragraph text-ancient-water',
                        contentTextClassName
                      )}
                    >
                      {bodyText}
                  ...
                  {showTips && season.tips ?
                    <ul className='grid gap-3'>
                      {season.tips.map((tip, index) => (
                        <li
                          key={index}
                          className='inspiration-seasons-tip-enter flex items-start gap-3 rounded-xl border border-cloud-dancer/10 bg-background/36 p-4'
                        ...
                        >
                          ...
                          <span className='leading-text-paragraph text-ancient-water'>{tip}</span>
                        </li>
                      ))}
                    </ul>
                  : null}
