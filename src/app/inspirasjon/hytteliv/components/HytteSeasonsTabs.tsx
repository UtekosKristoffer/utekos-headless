...
export function HytteSeasonsTabs() {
  ...
  return (
    <Tabs defaultValue='autumn' className='mx-auto max-w-4xl' onValueChange={setActiveTab}>
      <TabsList className='grid h-auto w-full grid-cols-4 gap-2 rounded-xl border border-background/12 bg-cloud-dancer p-1 shadow-[0_18px_46px_-36px_rgba(14,18,35,0.55)]'>
        {seasons.map(season => {
          ...
          return (
            <TabsTrigger
              key={season.value}
              value={season.value}
              className='hytte-seasons-tab-trigger relative min-h-12 overflow-hidden rounded-lg border border-background/12 px-3 transition-all duration-300 data-[state=active]:border-havdyp/24'
              style={{
                background:
                  isActive ? 'var(--havdyp)' : (
                    'color-mix(in oklch, var(--cloud-dancer) 82%, var(--overcast))'
                  ),
                color: isActive ? 'var(--cloud-dancer)' : 'var(--background)'
              }}
            >
              {isActive && (
                <div
              ...
              <div className='relative flex items-center justify-center gap-2 py-3'>
                <Icon
                  className={`size-5 transition-colors ${isActive ? season.iconColor : 'text-background'}`}
                />
                <span
                  className={`leading-[1] font-medium tracking-[-0.01em] transition-colors ${isActive ? 'text-cloud-dancer' : 'text-background'}`}
                >
                  {season.label}
                </span>
              </div>
        ...
        return (
          <TabsContent key={season.value} value={season.value} className='mt-8'>
            <div className='hytte-seasons-tab-content-enter'>
              <Card
                className='relative overflow-hidden border-background/14 py-0 text-background shadow-[0_28px_72px_-46px_rgba(14,18,35,0.58)]'
                style={{
              ...
              >
                ...
                <CardContent className='relative p-8'>
                  <div className='mb-6 flex items-center gap-4'>
                    <div
                      className='flex size-12 items-center justify-center rounded-lg border border-background/14 bg-cloud-dancer transition-shadow duration-300'
                      style={{
                    ...
                    <h3 className='text-2xl leading-[1] font-semibold tracking-[-0.01em] text-background'>
                      {season.title}
                    </h3>
                  </div>
