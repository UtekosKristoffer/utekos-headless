...
import { FALLBACK_SUMMARIES, MINIMUM_THINKING_TIME_MS } from '../constants'
import type {
...
export function NbccAiSummaryButton({
  ...
  const [status, setStatus] = useState<NbccAiSummaryStatus>('idle')
  ...
  async function handleActivate() {
    ...
    try {
      const minimumDelay = new Promise(resolve => setTimeout(resolve, MINIMUM_THINKING_TIME_MS))
      const fetchPromise = fetch(`/api/nbcc-ai-summary?intent=${encodeURIComponent(intent)}`, {
        cache: 'no-store'
      })
      ...
      >
        <div className='w-full overflow-hidden'>
          <div className='mt-3 w-full overflow-hidden rounded-xl border border-[#f0c36a]/25 bg-[#16120e] text-left shadow-xl shadow-black/20'>
            {isThinking ?
              <div aria-live='polite' aria-busy='true' className='px-5 py-5 text-[#f5efe4]'>
                <div className='flex items-center gap-3'>
                  <BrainCircuitIcon className='h-5 w-5 animate-pulse text-[#f0c36a]' aria-hidden />
                  <div>
                    <p className='text-[11px] font-bold uppercase tracking-[0.18em] text-[#f0c36a]'>
                      Henter veiledning
                    </p>
                    <p className='mt-1 text-sm text-[#f5efe4]/75'>Setter sammen en ryddig forklaring...</p>
                  </div>
                </div>
            ...
            : payload ?
              <>
                ...
                <div className='px-5 py-5'>
                  <p className='text-sm leading-7 text-[#f5efe4]/88'>{payload.intro}</p>
