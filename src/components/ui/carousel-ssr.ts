...
export const CAROUSEL_SSR = {
  ...
  responsiveThirds: (slideCount: number): SsrOptionsType => ({
    slideSizes: fillSlideSizes(slideCount, 100),
    breakpoints: {
      '(min-width: 768px)': { slideSizes: fillSlideSizes(slideCount, 50) },
      '(min-width: 1024px)': { slideSizes: fillSlideSizes(slideCount, 100 / 3) }
    }
  }),
  ...
  productGrid: (slideCount: number): SsrOptionsType => ({
    slideSizes: fillSlideSizes(slideCount, 100),
    breakpoints: {
      '(min-width: 640px)': { slideSizes: fillSlideSizes(slideCount, 50) },
      '(min-width: 768px)': { slideSizes: fillSlideSizes(slideCount, 100 / 3) },
      '(min-width: 1280px)': { slideSizes: fillSlideSizes(slideCount, 25) }
    }
  }),

  /** `basis-full` → `sm:basis-1/2` → `lg:basis-1/3` */
  responsiveHalvesAndThirds: (slideCount: number): SsrOptionsType => ({
    slideSizes: fillSlideSizes(slideCount, 100),
    breakpoints: {
      '(min-width: 640px)': { slideSizes: fillSlideSizes(slideCount, 50) },
      '(min-width: 1024px)': { slideSizes: fillSlideSizes(slideCount, 100 / 3) }
