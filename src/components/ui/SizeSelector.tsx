...
function SizeSelector({ optionName, values, variants, selectedVariant, onSelect }: SizeSelectorProps) {
  const [isDetailsOpen, setIsDetailsOpen] = useState(false)

  return (
    <div className='space-y-3'>
      {values.map(sizeValue => {
        const representativeVariant = variants.find(v => v.selectedOptions.some(opt => opt.name === optionName && opt.value === sizeValue))
        ...
        const isSelected = selectedVariant.selectedOptions.some(opt => opt.name === optionName && opt.value === sizeValue)

        return (
          <OptionButton key={sizeValue} isSelected={isSelected} onClick={() => onSelect(optionName, sizeValue)}>
      ...
      <div className='flex w-full flex-col rounded-lg border border-white/20 bg-surface-raised/40 p-4 text-left transition-colors'>
        <button onClick={() => setIsDetailsOpen(!isDetailsOpen)} className='flex w-full justify-between bg-transparent p-0 font-semibold text-foreground-on-dark/80 transition-colors hover:text-foreground-on-dark' aria-expanded={isDetailsOpen} aria-controls='size-details'>
          <span className='font-semibold'>Usikker på størrelsen?</span>
          <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' className={`transform transition-transform duration-200 ${isDetailsOpen ? 'rotate-45' : ''}`} aria-hidden='true'>
            <path d='M5 12h14' />
            <path d='M12 5v14' />
          </svg>
        </button>
        ...
        >
          <p className='text-xs text-foreground-on-dark/80'>
            ...
            <Link href='/størrelsesguide' className='underline hover:text-foreground-on-dark'>
              her
            </Link>
            .
          </p>
