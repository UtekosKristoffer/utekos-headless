 ...
 */
const getCartTriggerAriaLabel = (itemCount: number): string => `Åpne handlekurv, ${itemCount} ${itemCount === 1 ? 'vare' : 'varer'}`

/**
 * Creates the cart open handler.
 * Separates event handling logic for better testability.
 */
const createCartOpenHandler = (store: typeof cartStore) => () => store.send({ type: 'OPEN' })
...
export function CartTrigger(): React.JSX.Element {
  ...
  const itemCount = calculateItemCount(optimisticLines.lines)
  const handleOpenCart = createCartOpenHandler(cartStore)

  return (
    <DrawerTrigger asChild>
      <Button variant='outline' size='default' aria-label={getCartTriggerAriaLabel(itemCount)} className='relative' onClick={handleOpenCart}>
        <ShoppingBagIcon className='size-6' />
        {itemCount > 0 && <span className='absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground'>{itemCount}</span>}
      </Button>
    </DrawerTrigger>
  )
}
