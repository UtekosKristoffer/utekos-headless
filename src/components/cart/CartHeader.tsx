 ...
 */
const calculateItemCount = (lines: Record<string, number>): number => Object.keys(lines).length
 ...
 */
const getCartTitle = (itemCount: number): string => `Handlekurv (${itemCount} ${getItemText(itemCount)})`

/**
 * Generates contextually appropriate description based on cart state.
 * Customer-facing text, so in Norwegian.
 */
const getCartDescription = (itemCount: number): string => (itemCount === 0 ? 'Din handlekurv er tom. Legg til produkter for å komme i gang.' : `Se gjennom ${getItemText(itemCount)} i handlekurven din nedenfor.`)

/**
 * Generates accessible aria-label for close button.
 * Customer-facing accessibility text, so in Norwegian.
 */
const getCloseButtonAriaLabel = (itemCount: number): string => `Lukk handlekurv med ${itemCount} ${getItemText(itemCount)}`
  ...
  return (
    <DrawerHeader className='flex items-center justify-between border-b'>
      <div className='min-w-0 flex-1'>
        <DrawerTitle className='text-lg font-medium leading-tight'>{getCartTitle(itemCount)}</DrawerTitle>
        <DrawerDescription className='text-sm leading-relaxed'>{getCartDescription(itemCount)}</DrawerDescription>
      </div>
      <DrawerClose asChild>
        <Button variant='ghost' size='icon' className='ml-4 shrink-0' aria-label={getCloseButtonAriaLabel(itemCount)}>
          <X className='size-5' />
