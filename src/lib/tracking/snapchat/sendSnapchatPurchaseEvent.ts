// Path: src/lib/tracking/snapchat/sendSnapchatPurchaseEvent.ts
...
export async function sendSnapchatPurchaseEvent({
  order,
  customer,
  redisData,
  contentIds
  ...
  try {
    ...
    const snapPayload = {
      ...
      action_source: 'WEB',
      user_data: {
        em: [hashAdData(customer.email)].filter(Boolean),
        ph: [hashAdData(customer.phone)].filter(Boolean),
        ...
        fn: [hashAdData(customer.firstName)].filter(Boolean),
        ln: [hashAdData(customer.lastName)].filter(Boolean),
        ct: [hashAdData(customer.city)].filter(Boolean),
        st: [hashAdData(customer.state)].filter(Boolean),
        zp: [hashAdData(customer.zip)].filter(Boolean),
        country: [hashAdData(customer.countryCode)].filter(Boolean)
      },
      custom_data: {
