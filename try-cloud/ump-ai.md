...
The ucp object contains protocol metadata: version, services, and capabilities. Payment configuration is a sibling—see Payment Architecture. The signing_keys array contains public keys (JWK format) used to verify signatures on webhooks and other authenticated messages from the business.
...
Prune orphaned extensions: Remove any capability where extends is set but the parent capability is not in the intersection.
...
Negotiation (Business → Platform): The business analyzes the cart and advertises available handlers. This tells the platform how to pay (e.g., "Use this specific payment credential provider endpoint with this public key").
