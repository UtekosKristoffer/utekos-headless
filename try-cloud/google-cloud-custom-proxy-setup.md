...
_(Instructions for manual deployment would be added here)_
...
2. Check if there are requests to other endpoints. Depending on the server-side
   ...
   endpoints.
3. Allow-list any other endpoints that are needed for your tagging setup.
...
> the tagging server. If you use Cloud Run, store the proxy endpoint credentials
> in Secret Manager. Secret Manager gives Cloud Run access to the secret at
> startup time and resolves the value. Learn how to configure secrets in Cloud
> Run.
...
- `http://diagnostics.example2.com:3131`,
  `https://diagnostics.example2.com:3131`
