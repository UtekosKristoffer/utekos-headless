...
additional properties and methods to interact with the user agent object from
the request.
...
- `name`: A string representing the browser's name, or `undefined` if not
  identifiable.
...
- `type`: A string representing the type of the device, such as `console`,
  `mobile`, `tablet`, `smarttv`, `wearable`, `embedded`, or `undefined`.
...
- `name`: A string representing the engine's name. Possible values include:
...
- `architecture`: A string representing the architecture of the CPU. Possible
  values include: `68k`, `amd64`, `arm`, `arm64`, `armhf`, `avr`, `ia32`,
  `ia64`, `irix`, `irix64`, `mips`, `mips64`, `pa-risc`, `ppc`, `sparc`,
  `sparc64` or `undefined`
