...
This page provides an overview of the advanced traffic management capabilities available for internal Application Load Balancers.
...
At a high level, load balancers provide traffic management by leveraging URL maps and backend services.
...
1.  It selects a backend service based on rules in a URL map.
2.  The backend service selects a backend instance based on policies defined in the backend service.
...
| **Outlier detection**       | Specifies the criteria for eviction of unhealthy backend VMs or endpoints.     |
