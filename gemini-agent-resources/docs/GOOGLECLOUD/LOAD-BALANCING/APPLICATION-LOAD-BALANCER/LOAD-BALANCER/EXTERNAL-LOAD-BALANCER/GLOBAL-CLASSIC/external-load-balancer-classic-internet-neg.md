...
This guide uses an example to show how to use an external backend (custom
origin) with a classic Application Load Balancer. An external backend is an
endpoint external to Google Cloud. Using an external backend with this load
balancer allows you to improve performance with Cloud CDN caching.

This guide shows how to configure a classic Application Load Balancer with a
Cloud CDN-enabled backend service that proxies requests to an external backend
server.
...
> documentation, including the limitations.
...
To allow requests from Google Cloud, you must allowlist the IP address ranges
that Google uses to send requests to external backends. Query the
...
The output shows the CIDR ranges to allow in your firewall. For example:
`ip4:34.96.0.0/20`.
...
5.  Select **Classic Application Load Balancer** and click **Next**.
6.  Click **Configure**.
...
- To check if Cloud CDN is serving responses, see
  [Viewing logs](https://cloud.google.com/cdn/docs/logging).
- To learn about cacheable content, see
  [Caching overview](https://cloud.google.com/cdn/docs/caching).
- To find Cloud CDN's points of presence, see
