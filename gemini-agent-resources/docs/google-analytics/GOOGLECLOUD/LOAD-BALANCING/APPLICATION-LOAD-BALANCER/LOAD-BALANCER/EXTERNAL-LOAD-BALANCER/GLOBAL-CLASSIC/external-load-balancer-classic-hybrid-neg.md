...
This page illustrates how to deploy the classic Application Load Balancer to load balance traffic to network endpoints that are on-premises or in other public clouds and are reachable by using hybrid connectivity.

Note: For regional external Application Load Balancer instructions, see Set up a regional external Application Load Balancer with hybrid connectivity. For global external Application Load Balancer instructions, see Set up a global external Application Load Balancer with hybrid connectivity.
