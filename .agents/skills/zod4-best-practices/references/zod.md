...
> If you have uncommonly strict constraints around bundle size, consider the Zod Mini package.
...
All schemas extend the `z.ZodType` base class, which in turn extends `z.$ZodType` from [`zod/v4/core`](./zod-core.md). All instances of `ZodType` implement the following methods:
