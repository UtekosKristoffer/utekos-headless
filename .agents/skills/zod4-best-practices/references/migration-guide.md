...
This migration guide lists the breaking changes in Zod 4 in order of highest to lowest impact. To learn more about the performance enhancements and new features of Zod 4, read the [release notes](./release-notes.md).
...
- [`ZodError`](#zoderror)
- [`z.number()`](#znumber)
- [`z.string()` updates](#zstring-updates)
- [`z.coerce` updates](#zcoerce-updates)
- [`.default()` updates](#default-updates)
- [`z.object()`](#zobject)
- [`z.nativeEnum()` deprecated](#znativeenum-deprecated)
- [`z.array()`](#zarray)
- [`z.promise()` deprecated](#zpromise-deprecated)
- [`z.function()`](#zfunction)
- [`.refine()`](#refine)
- [`z.ostring()`, etc dropped](#zostring-etc-dropped)
- [`z.literal()`](#zliteral)
- [Static `.create()` factories dropped](#static-create-factories-dropped)
- [`z.record()`](#zrecord)
- [`z.intersection()`](#zintersection)
- [Internal changes](#internal-changes)

<Callout>
  **Note** — Zod 3 exported a number of undocumented quasi-internal utility
  types and functions that are not considered part of the public API. Changes to
  those are not documented here.
</Callout>

<Callout>
  **Unofficial codemod** — A community-maintained codemod
  [`zod-v3-to-v4`](https://github.com/nicoespeon/zod-v3-to-v4) is available.
</Callout>
...
The `.format()` method on `ZodError` has been deprecated. Instead use the top-level `z.treeifyError()` function. Read the [Formatting errors docs](./customizing-errors.md) for more information.

### deprecates `.flatten()`

The `.flatten()` method on `ZodError` has also been deprecated. Instead use the top-level `z.treeifyError()` function. Read the [Formatting errors docs](./customizing-errors.md) for more information.
...
> **Note**: For even better TypeScript performance, consider using object destructuring instead of `.extend()`. See the [Defining schemas](./defining-schemas.md#extend) docs for more details.
...
> If you are using `z.promise` to define an async function with `z.function()`, that's no longer necessary either; see the [`ZodFunction`](#zfunction) section below.
...
Refer to the [Zod Core](./zod-core.md) docs for more information on the contents of the core sub-library.
