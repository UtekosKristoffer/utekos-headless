...
Use utilities like `ease-in` and `ease-out` to control the easing curve of an
...
Use the `ease-[<value>]` syntax to set the transition timing function based on a
...
This is just a shorthand for `ease-[var(<custom-property>)]` that adds the
...
Prefix a `transition-timing-function` utility with a breakpoint variant like
`md:` to only apply the utility at medium screen sizes and above:
...
Use the `--ease-*` theme variables to customize the transition timing function
