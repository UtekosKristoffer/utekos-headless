...
Use utilities like `backdrop-saturate-50` and `backdrop-saturate-100` utilities
to control the saturation of an element's backdrop:
...
Use the `backdrop-saturate-[<value>]` syntax to set the backdrop saturation
...
This is just a shorthand for `backdrop-saturate-[var(<custom-property>)]` that
...
Prefix a `backdrop-filter: saturate()` utility with a breakpoint variant like
