...
Use utilities like `backdrop-sepia` and `backdrop-sepia-50` to control the sepia
effect applied to an element's backdrop:
...
Use the `backdrop-sepia-[<value>]` syntax to set the backdrop sepia based on a
...
For CSS variables, you can also use the `backdrop-sepia-(<custom-property>)`
syntax:
...
This is just a shorthand for `backdrop-sepia-[var(<custom-property>)]` that adds
...
Prefix a `backdrop-filter: sepia()` utility with a breakpoint variant like `md:`
