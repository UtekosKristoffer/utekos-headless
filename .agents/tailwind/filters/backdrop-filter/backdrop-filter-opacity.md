...
Use utilities like `backdrop-opacity-50` and `backdrop-opacity-75` to control
the opacity of all the backdrop filters applied to an element:
...
Use the `backdrop-opacity-[<value>]` syntax to set the backdrop filter opacity
...
For CSS variables, you can also use the `backdrop-opacity-(<custom-property>)`
syntax:
...
This is just a shorthand for `backdrop-opacity-[var(<custom-property>)]` that
...
Prefix a `backdrop-filter: opacity()` utility with a breakpoint variant like
