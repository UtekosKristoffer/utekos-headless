...
Use utilities like `backdrop-contrast-50` and `backdrop-contrast-100` to control
an element's backdrop contrast:
...
Use the `backdrop-contrast-[<value>]` syntax to set the backdrop contrast based
...
For CSS variables, you can also use the `backdrop-contrast-(<custom-property>)`
syntax:
...
This is just a shorthand for `backdrop-contrast-[var(<custom-property>)]` that
...
Prefix a `backdrop-filter: contrast()` utility with a breakpoint variant like
