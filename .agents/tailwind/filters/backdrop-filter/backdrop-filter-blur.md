...
Use the `backdrop-blur-[<value>]` syntax to set the backdrop blur based on a
...
This is just a shorthand for `backdrop-blur-[var(<custom-property>)]` that adds
...
Prefix a `backdrop-filter: blur()` utility with a breakpoint variant like `md:`
...
Use the `--blur-*` theme variables to customize the backdrop blur utilities in
your project:
