...
function stopTunnel() {
  ...
  for (const pid of pids) {
    ...
    if (!commandLine.includes('tunnel-client run') || !commandLine.includes(profile)) {
      console.log(`SKIP pid ${pid}: ${commandLine}`)
      continue
    }
...
function usage() {
  console.log('Usage: node scripts/mcp/openai-tunnel.mjs <bootstrap-env|check|init|doctor|gateway|run|stop|list-targets> [--target insight|browser|live-ops|commerce-tracking]')
}
