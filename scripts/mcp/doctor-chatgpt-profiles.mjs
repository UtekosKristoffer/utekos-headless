...
function printChecks(checks) {
  for (const item of checks) {
    const label = item.level === 'ok' ? 'OK' : item.level === 'warn' ? 'WARN' : 'ERROR'
...
function checkProfile(checks, profile) {
  ...
  const missingServers = profile.servers.filter(server => !existingServers.has(server))

  check(
    checks,
    `profile:${profile.id}:servers`,
    missingServers.length === 0 ? 'ok' : 'error',
    missingServers.length === 0 ? `${existingServers.size} servers` : `missing ${missingServers.join(', ')}`
  )

  const dryRun = run('docker', ['mcp', 'gateway', 'run', '--profile', profile.id, '--dry-run'])
  check(
    checks,
    `profile:${profile.id}:dry-run`,
    dryRun.status === 0 ? 'ok' : 'error',
    dryRun.status === 0 ? 'gateway dry-run OK' : firstLine(dryRun.stderr) || firstLine(dryRun.stdout) || 'failed'
  )

  const tools = run('docker', ['mcp', 'tools', 'ls', `--gateway-arg=--profile=${profile.id}`, '--format=list'])
  if (tools.status !== 0) {
    check(checks, `profile:${profile.id}:tools`, 'error', firstLine(tools.stderr) || 'tool list failed')
    return
  }

  const toolNames = parseTools(tools.stdout)
  check(checks, `profile:${profile.id}:tool-count`, 'ok', `${toolNames.size} tools`)
  ...
  for (const toolName of profile.forbiddenTools ?? []) {
    const unqualified = toolName.includes('.') ? toolName.split('.').at(-1) : toolName
    check(
      checks,
  ...
  for (const server of profile.directOnlyServers ?? []) {
    check(checks, `profile:${profile.id}:direct-only:${server}`, 'warn', 'not Docker-catalog backed in V1')
  }
...
function checkSecretPosture(checks, config) {
  const unsafeProfiles = config.profiles.filter(profile => {
    const filesystemPaths = profile.config?.['filesystem.paths'] ?? []
    return profile.mode !== 'explicit-write' && filesystemPaths.includes('${repoRoot}')
  })
  ...
  check(
    ...
    unsafeProfiles.length === 0 ? 'default profiles do not mount full repo root' : `unsafe profiles: ${unsafeProfiles.map(profile => profile.id).join(', ')}`
  )
}
...
function main() {
  ...
  check(checks, 'docker:mcp', docker.status === 0 ? 'ok' : 'error', docker.status === 0 ? firstLine(docker.stdout) : firstLine(docker.stderr) || 'unavailable')
  ...
  if (config.globalDockerFeatures?.['dynamic-tools'] === false) {
    check(
      ...
      dynamicTools === false ? 'ok' : dynamicTools === true ? 'error' : 'warn',
      dynamicTools === false ? 'disabled' : dynamicTools === true ? 'enabled; run npm run mcp:chatgpt:apply' : 'status unavailable'
    )
  }
  ...
  if (errors.length > 0) {
    console.error(`mcp:chatgpt:doctor failed with ${errors.length} error(s)`)
    process.exit(1)
  }

  console.log(`mcp:chatgpt:doctor OK${warnings.length > 0 ? ` with ${warnings.length} warning(s)` : ''}`)
}
